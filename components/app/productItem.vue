   ...
   <div v-if="product" class="flex border rounded my-2 bg-white hover:shadow-md shadow-green-600 cursor-pointer overflow-hidden">
        ...
        <div class="flex-1 flex flex-col">
            <div class="flex relative">
                <div v-if="authStore.getToken()" :class="`bg-[${colorTheme}] absolute top-0 right-0 z-50 rounded-bl w-8 h-8 flex items-center justify-center`"><n-icon><Brush class="text-white animate-bounce" /></n-icon></div>
                <div class="w-24 h-24 overflow-hidden">
                    <NuxtImg :src="product.image" alt="imagem do produto" class="w-full h-full object-cover"/>
                </div>
